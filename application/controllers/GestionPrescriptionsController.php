...
class GestionPrescriptionsController extends Zend_Controller_Action
{
    ...
    public function showprescriptiontexteAction(): void
    {
        ...
        if ($this->getRequest()->getParam('id')) {
            ...
            $idCategorie = $this->getRequest()->getParam('id');
            ...
            $this->view->assign('prescriptionType', $service_prescription->showPrescriptionType($this->getRequest()->getParam('id'), 0, 0));
        }
    }
    ...
    {
        $service_prescription = new Service_Prescriptions();
        if ($this->getRequest()->getParam('idTexte')) {
            $this->view->assign('idTexte', $this->getRequest()->getParam('idTexte'));
            // ici on affiche les textes appartenant à la catégorie dont on passe l'id en param
            $idTexte = $this->getRequest()->getParam('idTexte');
            ...
            $this->view->assign('prescriptionType', $service_prescription->showPrescriptionType($idCategorie, $this->getRequest()->getParam('idTexte'), 0));
        }
    }
    ...
    public function showarticlecontenuAction(): void
    {
        ...
        $service_prescription = new Service_Prescriptions();
        if ($this->getRequest()->getParam('idArticle')) {
            $dbArticle = new Model_DbTable_PrescriptionArticle();
            $article = $dbArticle->find($this->getRequest()->getParam('idArticle'))->current();
            $dbTexte = new Model_DbTable_PrescriptionTexte();
            $texte = $dbTexte->find($article['ID_PRESCRIPTIONTEXTE'])->current();
            $this->view->assign('prescriptionType', $service_prescription->showPrescriptionType($texte['ID_PRESCRIPTIONCAT'], $article['ID_PRESCRIPTIONTEXTE'], $this->getRequest()->getParam('idArticle')));
            $this->view->assign('idArticle', $this->getRequest()->getParam('idArticle'));
        }
    }
    ...
    {
        if ($this->getRequest()->getParam('id')) {
            $this->view->assign('idCategorie', $this->getRequest()->getParam('id'));
            $dbCat = new Model_DbTable_PrescriptionCat();
            $this->view->assign('catInfo', $dbCat->find($this->getRequest()->getParam('id'))->current());
        }
    }
    ...
    public function savecategorieAction(): void
    {
        try {
            ...
            if ($this->getRequest()->getParam('idCat')) {
                // Edition
                $this->view->assign('do', 'edit');
                $categorie = $dbCat->find($this->getRequest()->getParam('idCat'))->current();
            } else {
                // Création
            ...
            $categorie->LIBELLE_PRESCRIPTION_CAT = $this->getRequest()->getParam('LIBELLE_PRESCRIPTION_CAT');
            $categorie->save();
    ...
    public function formtexteAction(): void
    {
        if ($this->getRequest()->getParam('idCat')) {
            ...
            $this->view->assign('idCategorie', $this->getRequest()->getParam('idCat'));
        } elseif ($this->getRequest()->getParam('idTexte')) {
            // edition d'un texte existant
            $this->view->assign('idTexte', $this->getRequest()->getParam('idTexte'));
            $dbTexte = new Model_DbTable_PrescriptionTexte();
            $this->view->assign('texteInfo', $dbTexte->find($this->getRequest()->getParam('idTexte'))->current());
        }
    }
    ...
    public function savetexteAction(): void
    {
        try {
            ...
            if ($this->getRequest()->getParam('idCategorie')) {
                // Sauvegarde d'un nouveau texte
                $this->view->assign('do', 'new');
                $texte = $dbTexte->createRow();
                // On recupere le num max de la catégorie
                $numMax = $dbTexte->recupMaxNumTexte($this->getRequest()->getParam('idCategorie'));
                ...
                $texte->ID_PRESCRIPTIONCAT = $this->getRequest()->getParam('idCategorie');
            } elseif ($this->getRequest()->getParam('idTexte')) {
                // edition d'un texte
                $this->view->assign('do', 'edit');
                $texte = $dbTexte->find($this->getRequest()->getParam('idTexte'))->current();
            }

            $texte->LIBELLE_PRESCRIPTIONTEXTE = $this->getRequest()->getParam('LIBELLE_PRESCRIPTIONTEXTE');
            $texte->save();
    ...
    public function formarticleAction(): void
    {
        if ($this->getRequest()->getParam('idTexte')) {
            // Création d'un nouveau texte
            $this->view->assign('idTexte', $this->getRequest()->getParam('idTexte'));
        } elseif ($this->getRequest()->getParam('idArticle')) {
            // edition d'un texte existant
            $this->view->assign('idArticle', $this->getRequest()->getParam('idArticle'));
            $dbArticle = new Model_DbTable_PrescriptionArticle();
            $this->view->assign('articleInfo', $dbArticle->find($this->getRequest()->getParam('idArticle'))->current());
        }
    }
    ...
    public function savearticleAction(): void
    {
        try {
            ...
            if ($this->getRequest()->getParam('idTexte')) {
                ...
                // On recupere le num max de la catégorie
                $numMax = $dbArticle->recupMaxNumArticle($this->getRequest()->getParam('idTexte'));
                ...
                $article->ID_PRESCRIPTIONTEXTE = $this->getRequest()->getParam('idTexte');
            } elseif ($this->getRequest()->getParam('idArticle')) {
            ...
            $article->LIBELLE_PRESCRIPTIONARTICLE = $this->getRequest()->getParam('LIBELLE_PRESCRIPTIONARTICLE');
            $article->save();
    ...
    public function formprescriptionAction(): void
    {
        ...
        $service_prescription = new Service_Prescriptions();
        if ($this->getRequest()->getParam('idPrescType')) {
            $this->view->assign('idPrescType', $this->getRequest()->getParam('idPrescType'));
            $this->view->assign('do', 'edit');

            $this->view->assign('assoc', $service_prescription->getPrescriptionTypeDetail($this->getRequest()->getParam('idPrescType')));
        } else {
            $this->view->assign('do', 'new');
            $dbCategorie = new Model_DbTable_PrescriptionCat();
            $this->view->assign('listeCategorie', $dbCategorie->recupPrescriptionCat());

            switch ($this->getRequest()->getParam('typePresc')) {
                case 'addPrescriptionCat':
                    // cas d'une prescription dans une catégorie
                    $this->view->assign('categorie', $this->getRequest()->getParam('empl'));
                ...
                case 'addPrescriptionTexte':
                    ...
                    $texteInfo = $dbPrescTexte->find($this->getRequest()->getParam('empl'))->current();
                    $this->view->assign('categorie', $texteInfo->ID_PRESCRIPTIONCAT);
                    $this->view->assign('texte', $this->getRequest()->getParam('empl'));
                ...
                case 'addPrescriptionArticle':
                    ...
                    $articleInfo = $dbPrescArticle->find($this->getRequest()->getParam('empl'))->current();
                    ...
                    $this->view->assign('article', $this->getRequest()->getParam('empl'));
    ...
    public function gestionTextesEditAction(): void
    {
        ...
        $service_prescTextes = new Service_Prescriptions();
        $texteInfo = $service_prescTextes->getTexte($this->getRequest()->getParam('id'));
    ...
    public function gestionArticlesEditAction(): void
    {
        ...
        $service_prescription = new Service_Prescriptions();
        $articleInfo = $service_prescription->getArticle($this->getRequest()->getParam('id'));
    ...
    public function gestionRappelRegEditAction(): void
    {
        ...
        $idPrescription = $this->getRequest()->getParam('id');
        $typeAction = 'rappel-reg';

        $service_prescription = new Service_Prescriptions();
        $prescriptionInfo = $service_prescription->getPrescriptionInfo($this->getRequest()->getParam('id'), $typeAction);
